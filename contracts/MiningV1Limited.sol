...
contract MiningV1Limited is OwnableUpgradeable {
    ...
    function claim(uint256 stakeId) external {
        require(msg.sender == miningV2, "Claim only via mining v2");
        require(block.number >= stakes[stakeId].endBlock, "Can't claim yet");
        require(!stakes[stakeId].claimed, "Stake is already claimed");

        address sender = stakes[stakeId].owner;

        stakes[stakeId].claimed = true;
        for (uint256 i = 0; i < stakeIds[sender].length; i++) {
            if (stakeIds[sender][i] == stakeId) {
                stakeIds[sender][i] = stakeIds[sender][stakeIds[sender].length - 1];
                stakeIds[sender].pop();
                break;
            }
        }
        ...
        grvx.mint(sender, stakes[stakeId].grvxReward);
        ...
        if (stakesExtra[stakeId].hasSkin) {
            skins.transferFrom(address(this), sender, stakesExtra[stakeId].skinId);
        }
        ...
        if (scrapQuantity > 0) {
            scrap.mint(sender, scrapQuantity * 10**18);
        }
    ...
    function setMiningV2(address miningV2_) external onlyOwner {
        miningV2 = miningV2_;
    }
